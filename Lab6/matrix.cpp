...
matrix matrix::operator+(const matrix& rhs) const{
	//Verify matrices match in size
	if(rows != rhs.rows || cols != rhs.cols)
		throw matrix_Exception("Size mismatch - The column/row of the left matrix"
                            " does not match the column/row of the right matrix:");
...
double* matrix::operator[](unsigned int row){
	...
	if (row >= rows || row < 0)
		throw matrix_Exception("Size mismatch - The requested row is"
								" out of bounds.");	
